...
def find_faces(files):
    i = 0
    for face in files:
        ret = face_detector(face)
        if(ret):
            i = i+1
    return i
...
# dog = find_faces(human_files_short)
# print("human_files_short have "+ str(dog)+"% faces")
# findHunman  = find_faces(dog_files_short)
# print("dog_files_short have "+ str(findHunman)+"% faces")
...
# 
# __回答:__我认为这并不合理，在实际应用中用户并不一定能得到清晰的人脸图像，我认为在实际应用中只要检测到面部的一部分就算检测到人脸，比如检测到人的眼睛，耳朵，鼻子等明显的可以区分是人的特征就可以认为检测到人脸。
# 
# 
...
def find_dogs(files):
    i = 0
    for dog in files:
        ret = dog_detector(dog)
        if(ret):
            i = i+1
    return i
    
# dog = find_dogs(dog_files_short)
# print("dog_files_short have "+ str(dog)+"% dogs")
# findHunman  = find_dogs(human_files_short)
# print("human_files_short have "+ str(findHunman)+"% dogs")
...
# 
# __回答:__ 我是根据上图提示步骤搭建的卷积网络，上图的卷积网络看起来很像有名的LeNet结构，但是做了改动
...
# 3，池化
# 4,全连接层分类
# 但是上图将卷积核大小换成了2，这样可以减少参数数量，将卷积和池化操作做了三次，卷积层可以提取特征映射，池化层可以降低维度，这些层合力提取出有用的足够的特征，然后用全连接层连接，全连接层利用这些特征进行分类从而得出我们需要的狗狗的分类。
# 综上，卷积和池化是特征提取器，全连接层是分类器，所以这个架构在图像分类上问题上是有效的。
...
model.add(Conv2D(filters=32, kernel_size=2, strides=1, padding='valid', activation='relu', input_shape=(111, 111, 3)))
model.add(MaxPooling2D(pool_size=2, input_shape=(110, 110, 3)))

model.add(Conv2D(filters=64, kernel_size=2, strides=1, padding='valid', activation='relu', input_shape=(55, 55, 3)))
model.add(MaxPooling2D(pool_size=2, input_shape=(54, 54, 3)))
...
# 
# 这里很明显属于第二种，已经提取了特征向量，接下来只要训练全连接层就好了，
# 这里使用GAP来替代全连接层，然后Dropout防止过拟合，最后分类就可以了。
# 
# 
...
def Xception_predict_breed(img_path):
    ...
    # 返回此模型预测的狗的品种
    return dog_names[np.argmax(predicted_vector)]
breed = Xception_predict_breed(human_files[3])
print("bread is "+breed)
...
### TODO: 设计你的算法
### 自由地使用所需的代码单元数吧
def judge_dog_detector(img_path):
    ...
    breed = "error"
    if (has_face or has_dog):
        breed = Xception_predict_breed(img_path)

    ret = "pic has " +classfiy + "\nbreed is:"+breed
    return ret

breed = judge_dog_detector(human_files[18])
print(breed)
...
pic_files = np.array(glob("testImg/*"))
# random.shuffle(pic_files)
# 打印数据集的数据量
print('There are %d total images.' % len(pic_files))

#不显示图像
def judge_dog_detector_noimg(img_path):
    has_face = face_detector(img_path)
    classfiy = "none"
    if has_face:
        classfiy = "human"
...
i = 0
for pic_path in pic_files[:8]:
    plt.subplot(4,4,i+1)     #将窗口分为两行两列四个子图，则可显示四幅图片
    plt.axis('off')
    im = np.array(Image.open(pic_path))
    plt.imshow(im)
    breed = judge_dog_detector_noimg(pic_path)
    plt.title(breed)
    i += 1
plt.show()
