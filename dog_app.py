...
def judge_dog_detector_noimg(img_path):
    ...
    ret = "pic has " +classfiy + "\nbreed is:"+breed
    return ret
# breed = judge_dog_detector(pic_files[0])
# print("bread is "+breed)
...
# 输出结果和预期的差不多，结果都正确
# 改进方法如下：
# 1做数据增强，可以将训练模型的数据使用datagen方法增加训练样本
# 2，测试单个模型VGG16、VGG19、ResNet50、InceptionV3、Xception，选择准确率最高的单个模型进行迁移学习
# 3，使用多个模型进行融合，使用多个预训练模型的特征向量融合后进行训练。
# 4，尝试多个组合进行融合训练后，选择准确率最高的组合来完成模型。
# 
